...
class VidCloud extends models_1.VideoExtractor {
    constructor() {
        ...
        this.extract = async (videoUrl, isAlternative = true) => {
            var _a;
            const result = {
                sources: [],
                subtitles: [],
            ...
            try {
                ...
                const options = {
                    headers: {
                        ...
                        'User-Agent': keys.agent || utils_1.USER_AGENT,
                    },
                };
                let res = undefined;
                let sources = undefined;
                console.log("res.data", `${isAlternative ? this.host2 : this.host}/ajax/embed-4/getSources?id=${id}&v=${keys.v}&h=${keys.h}&b=${keys.b}`);
                res = await this.client.get(`${isAlternative ? this.host2 : this.host}/ajax/v2/embed-4/getSources?id=${id}&v=${keys.v}&h=${keys.h}&b=${keys.b}`, options);
                console.log("res.data", res.data);
                if (false && !(0, utils_1.isJson)(res.data.sources)) {
